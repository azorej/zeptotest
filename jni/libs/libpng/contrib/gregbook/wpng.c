...
int main(int argc, char **argv)
{
    ...
    if ((p = getenv("SCREEN_GAMMA")) != NULL)
    {
        ...
        if (exponent > 0.0) default_gamma = 1.0 / exponent;
    }

    if (default_gamma == 0.0) default_gamma = 1.0 / default_display_exponent;
    ...
    while (*++argv && !error)
    {
        ...
        else if (!strncmp(*argv, "-gamma", 2))
        {
            ...
            else
            {
                ...
                if (wpng_info.gamma <= 0.0) ++error;
                else if (wpng_info.gamma > 1.01) fprintf(stderr, PROGNAME
    ...
    if (!error)
    {
        ...
        else
        {
            ...
            if (wpng_info.pnmtype != 8) wpng_info.have_bg = FALSE; /* no need for bg if opaque */
            do
            {
            ...
            (textbuf = (char *) malloc((5 + 9) * 75)) != NULL)
    {
        ...
        do
        {
            ...
            for (i = 1; i < 10; ++i)
            {
                fprintf(stderr, "    [%d] ", i);
                fflush(stderr);
                if (FGETS(p, 74, keybd) && (len = strlen(p)) > 1) p += len; /* now points at NULL; char before is newline */
                else
                    break;
            }
    ...
    if (wpng_info.pnmtype == 5) rowbytes = wpng_info.width;
    else if (wpng_info.pnmtype == 6) rowbytes = wpng_info.width * 3;
    else
    ...
    if (wpng_info.interlaced)
    {
        ...
        wpng_info.row_pointers = (uch **) malloc(
                wpng_info.height * sizeof(uch *));
...
static int wpng_isvalid_latin1(uch *p, int len)
{
    ...
    for (i = 0; i < len; ++i)
    {
        if (p[i] == 10 || (p[i] > 31 && p[i] < 127) || p[i] > 160) continue; /* character is completely OK */
        if (result < 0 || (p[result] != 27 && p[i] == 27)) result = i; /* mark location of first questionable one */
