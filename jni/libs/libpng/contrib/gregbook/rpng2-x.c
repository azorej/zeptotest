...
int main(int argc, char **argv)
{
    ...
    if ((p = getenv("SCREEN_GAMMA")) != NULL) rpng2_info.display_exponent =
            atof(p);
    else
    ...
    while (*++argv && !error)
    {
        ...
        else if (!strncmp(*argv, "-gamma", 2))
        {
            ...
            else
            {
                ...
                if (rpng2_info.display_exponent <= 0.0) ++error;
            }
        }
        else if (!strncmp(*argv, "-bgcolor", 4))
        {
            ...
            {
                bgstr = *argv;
                if (strlen(bgstr) != 7 || bgstr[0] != '#') ++error;
                else
                {
                    have_bg = TRUE;
                    bg_image = FALSE;
    ...
    if (alen + flen + 3 > 1023) sprintf(titlebar, "%s:  ...%s", appname,
    ...
    for (;;)
    {
        ...
        if (readpng2_decode_data(&rpng2_info, inbuf, incount)) ++error;
...
static void rpng2_x_init(void)
{
    ...
    rpng2_info.row_pointers = (uch **) malloc(
            rpng2_info.height * sizeof(uch *));
...
static int rpng2_x_create_window(void)
{
    ...
    if (depth != 16 && depth != 24 && depth != 32)
    {
        ...
        if (visuals_matched == 0)
        {
            ...
            return 2;
                ...
                (stderr, "XGetVisualInfo() returned %d 24-bit visuals\n", visuals_matched))
    ...
    if (depth == 8 || need_colormap)
    {
        ...
        have_colormap = TRUE;
        if (depth == 8) bg_image = FALSE; /* gradient just wastes palette entries */
    }
    ...
    if (depth == 8) XSetWindowColormap(display, window, colormap);

    if (!XStringListToTextProperty(&window_name, 1, pWindowName)) pWindowName =
            NULL;
    ...
    if (pWindowName) XFree(pWindowName->value);
    if (pIconName) XFree(pIconName->value);
    ...
    if (bg_image) rpng2_x_load_bg_image(); /* resets bg_image if fails */

    if (!bg_image)
    {
        if (depth == 24 || depth == 32)
        {
            bg_pixel = (bg_red << RShift) | (bg_green << GShift)
                    | (bg_blue << BShift);
        }
        else if (depth == 16)
        {
...
static int rpng2_x_load_bg_image(void)
{
    ...
    else if ((bg[pat].type & 0x07) == 1)
    {
        ...
        {
            yidx = (int) (row % bgscale);
            if (yidx > hmax) yidx = bgscale - 1 - yidx;
            dest = (char *) bg_data + row * bg_rowbytes;
            for (i = 0; i < rpng2_info.width; ++i)
            {
                xidx = (int) (i % bgscale);
                if (xidx > hmax) xidx = bgscale - 1 - xidx;
                k = xidx + yidx;
    ...
    else if ((bg[pat].type & 0x07) == 2)
    {
        ...
        for (row = 0; row < rpng2_info.height; ++row)
        {
            ...
            for (i = 0; i < rpng2_info.width; ++i)
            {
                ...
                {
                    if ((hue < 0.0) || (hue >= 360.0)) hue -= (((int) (hue
                            / 360.0)) * 360.0);
                    hue /= 60.0;
                    ii = (int) hue;
                    f = hue - (double) ii;
...
static void rpng2_x_display_row(ulg row)
{
    ...
    if (rows == 0) firstrow = row; /* first row that is not yet displayed */
    ...
    if (depth == 24 || depth == 32)
    {
        ...
        if (bg_image) src2 = bg_data + row * bg_rowbytes;
        dest = ximage->data + row * ximage_rowbytes;
        ...
        ulg height)
{
    ...
    Trace(
            ...
            (stderr, "   (byte_order = %s)\n", ximage->byte_order == MSBFirst? "MSBFirst" : (ximage->byte_order == LSBFirst? "LSBFirst" : "unknown")))
