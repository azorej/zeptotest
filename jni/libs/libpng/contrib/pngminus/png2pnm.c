        ...
        BOOL alpha)
{
    ...
    if ((png_pixels = (png_byte *) malloc(row_bytes * height * sizeof(png_byte)))
            == NULL)
    {
        png_destroy_read_struct(&png_ptr, &info_ptr, NULL);
        return FALSE ;
    ...
    for (row = 0; row < height; row++)
    {
        for (col = 0; col < width; col++)
        {
            ...
            {
                if (raw) fputc((int) *pix_ptr++, pnm_file);
            ...
            if (alpha_present)
            {
                ...
                {
                    pix_ptr++; /* alpha */
                    if (bit_depth == 16) pix_ptr++;
                }
                else /* output alpha-channel as pgm file */
                {
                    if (raw) fputc((int) *pix_ptr++, alpha_file);
                    else if (bit_depth == 16)
                    {
                        dep_16 = (long) *pix_ptr++;
                        fprintf(alpha_file, "%ld ",
            ...
            if (!raw) if (col % 4 == 3) fprintf(pnm_file, "\n");
        } /* end for col */
