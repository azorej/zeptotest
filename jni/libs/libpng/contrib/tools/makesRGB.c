...
int main(int argc, char **argv)
{
    ...
    if (argc > 1) test_only = strcmp("--test", argv[1]) == 0;
    ...
    for (;;)
    {
        if (ec_lo == 0) adjust = adjust_lo;

        else if (ec_hi == 0) adjust = adjust_hi;

        else if (ec_mid == 0) adjust = adjust_mid;

        else if (ec_mid < ec_hi) adjust = (adjust_mid + adjust_hi) / 2;

        else if (ec_mid < ec_lo) adjust = (adjust_mid + adjust_lo) / 2;
        ...
        for (i16 = 0; i16 <= 65535; ++i16)
        {
            ...
            if (icalc != iexact) ++error_count16;
        }

        /* Now try changing the adjustment. */
        if (ec_lo == 0) ec_lo = error_count16;

        else if (ec_hi == 0) ec_hi = error_count16;
    ...
    for (ibase = 0; ibase < 65536; ibase += 128)
    {
        ...
        png_byte delta = png_sRGB_delta[ibase >> 7], trydelta = delta, od =
                delta;
        ...
        for (;;)
        {
            ...
            {
                if (trybase > base) trybase = base - 1;
                else if (trybase < base)
                {
                    trybase = base;
                    ++trydelta;
                }
                else if (trydelta > delta) trydelta = delta - 1;
                else if (trydelta < delta) break; /* end of tests */
            }
        }
        ...
        else if (0) printf("/* table[%u]={%u,%u} %u errors */\n", ibase >> 7,
                ob, od, ecbase);
    }
    ...
    for (i = 0; i <= max_input; ++i)
    {
        ...
        if (icalc != iexact)
        {
            ...
            else if (err < min_error) min_error = err;
        }
    }
    ...
    for (i16 = 0; i16 <= 65535; ++i16)
    {
        ...
        if (icalc != iexact)
        {
            ...
            ++error_count16;
            if (err > max_error16) max_error16 = err;
            else if (err < min_error16) min_error16 = err;

            if (abs(icalc - iexact) > 1) printf(
