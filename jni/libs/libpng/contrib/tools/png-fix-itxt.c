...
;
        ...
        {
            if (length >= MAX_LENGTH - 12) break; /* To do: handle this more gracefully */
...
;
            ...
            for (;;)
            {
                ...
                if (((crc >> 24) & 0xff) == buf[length + 8]
                        ...
                        && ((crc) & 0xff) == buf[length + 11]) break;

                length++;

                if (length >= MAX_LENGTH - 12) break;
...
;                putchar(c);
            ...
            if (buf[4] == 73 && buf[5] == 69 && buf[6] == 78 && buf[7] == 68) break;
        }
