...
#ifndef ZEPTOTEST_VECMATH_HPP_
#define ZEPTOTEST_VECMATH_HPP_
...
public:
	...
	mat3& postTranslate(float tx, float ty) {
		f[6] += (tx * f[0]) + (ty * f[3]);
		f[7] += (tx * f[1]) + (ty * f[4]);
		f[8] += (tx * f[2]) + (ty * f[5]);
		return *this;
...
// Misc
//--------------------------------------------------------------------------------
	static mat3 view_to_world(float coordinate_factor, float aspect_ratio);
	static mat3 translation(const float fX, const float fY);
	static mat3 translation(const vec2& vec);
	static mat3 rotation(const float angle);
	static mat3 scaling(float fX, float fY) {
		mat3 ret;
		ret.f[0] = fX;
		ret.f[1] = 0;
		ret.f[2] = 0;
...
//--------------------------------------------------------------------------------
	...
	static mat4 scaling(float fX, float fY, float fZ) {
		mat4 ret;
		ret.f[0] = fX;
		ret.f[1] = 0;
		ret.f[2] = 0;
...
//Non destuctive version
	...
	void toMatrix(mat4& mat) {
		...
		mat.f[9] = yz - xw;
		mat.f[10] = 1.0f - x2 - y2;
		mat.f[3] = mat.f[7] = mat.f[11] = mat.f[12] = mat.f[13] = mat.f[14] =
				0.0f;
		mat.f[15] = 1.0f;
	}
