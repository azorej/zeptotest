...
void fill_shader_t::set_vertices(GLfloat* array, size_t size)
{
    _indices_count = size / 2;

    glBindBuffer(GL_ARRAY_BUFFER, _vbo);
    glBufferData(GL_ARRAY_BUFFER, (GLsizeiptr) (size * sizeof(GLfloat)), array, GL_DYNAMIC_DRAW);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
