...
namespace subsystems
{
    ...
    void graphic_t::start()
    {
        ...
        _redraw_task = application_t::singleton().subsystem<subsystems::scheduler_t>().schedule_repeat(
                std::bind(&subsystems::graphic_t::draw, this), interval);
    }
    ...
    void graphic_t::init_context()
    {
        ...
        my_assert((_display != EGL_NO_DISPLAY) && "Couldn't get default display");

        auto res = eglInitialize(_display, 0, 0);
        my_assert((res == EGL_TRUE) && "Couldn't init egl");

        /* Here, the application chooses the configuration it desires. In this
         * sample, we have a very simplified selection process, where we pick
         * the first EGLConfig that matches our criteria */
        res = eglChooseConfig(_display, config_attribs, &config, 1, &numConfigs);
        my_assert((res == EGL_TRUE) && "Couldn't get display configs");
        ...
        my_assert((res == EGL_TRUE) && "Couldn't get display config attrib");

        ANativeWindow_setBuffersGeometry(_java_app->window, 0, 0, format);

        _surface = eglCreateWindowSurface(_display, config, _java_app->window,
        NULL);
        my_assert((_surface != EGL_NO_SURFACE) && "Couldn't create window surface");
        _context = eglCreateContext(_display, config, NULL, context_attribs);
        my_assert((_context != EGL_NO_CONTEXT) && "Couldn't create context");

        res = (eglMakeCurrent(_display, _surface, _surface, _context) != EGL_FALSE);
        my_assert((res != EGL_FALSE) && "Unable to eglMakeCurrent");
